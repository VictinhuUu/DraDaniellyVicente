...
async function loginComGoogle() {
  ...
  try {
    ...
    console.error("Erro ao fazer login via popup:", error.code, error.message);

    // Popup bloqueado, fechado pelo usuário antes da hora, ou não
    // suportado no navegador → tenta redirect como alternativa.
    if (
      ...
      try {
        ...
        console.error("Erro ao fazer login via redirect:", redirectError.code, redirectError.message);
        exibirErro("Não foi possível entrar com o Google. Verifique se pop-ups estão liberados para este site.");
      }
      return;
    }

    // Usuário fechou o popup por conta própria: não é um erro real,
    // não precisa mostrar mensagem de erro assustando ninguém.
...
/**
 * Finaliza o fluxo de signInWithRedirect, para os casos em que o
 * login caiu no fallback de redirect (ver loginComGoogle()).
 *
 * Este passo é necessário porque, quando o usuário volta do Google
 * via redirect, o Firebase precisa que getRedirectResult() seja
 * chamado para concluir a autenticação e persistir a sessão —
 * especialmente em navegadores mobile que restringem armazenamento
 * entre contextos de terceiros. Sem essa chamada, o login pode
 * "acontecer" no Google mas nunca se consolidar no seu site.
 */
